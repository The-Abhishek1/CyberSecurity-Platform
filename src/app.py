...
async def init_orchestrator_components():
    ...
    scanner_agent = ScannerAgent(tool_router, memory_service)
    recon_agent = ReconAgent(tool_router, memory_service)
    ...
    await orchestrator.register_domain_agent("scanner", scanner_agent)
    await orchestrator.register_domain_agent("recon", recon_agent)
    
    # Initialize worker pools for each tool
    tools = await tool_registry.list_tools()
    for tool in tools:
        await worker_pool.initialize_pool(tool["name"], tool)
...
def create_app() -> FastAPI:
    ...
    app.add_middleware(AuthenticationMiddleware, exclude_paths=["/health", "/metrics", "/docs", "/redoc", "/openapi.json"])
